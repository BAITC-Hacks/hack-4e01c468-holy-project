...
Основные каталоги: `wind_forecast/` — Python-приложение и адаптеры, `frontend/` — Astro-интерфейс, `tests/` и `frontend/tests/` — проверки, `docs/` — документация. `artifacts/` и `.cache/` содержат локальные результаты и погодный кэш и не входят в Git. `windline-lumen-demo/` сохранён как исходный дизайн-референс; рабочий интерфейс находится в `frontend/`.
